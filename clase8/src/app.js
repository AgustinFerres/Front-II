...
window.addEventListener('load', function(){
  ...
  const nodoNoticias = document.querySelectorAll('.contenedor-main section article')
  
  
  nodoNoticias.forEach((noticia, i) => {


    /* -------------------------------- slectores ------------------------------- */

    const nodoTitulo = document.querySelector('h3');
    const nodoImagen = document.querySelector('img');
    const nodoDescripcion = document.querySelector('p');
    const nodoFecha = document.querySelector('.fecha');
    ...
    <p class='fecha'>${noticias[i].fecha}</p>
    `
  })
});